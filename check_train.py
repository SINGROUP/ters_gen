# Training specific model (no hyperparameter tuning)
...
def objective( config, device):

    # Sample hyperparameters
    batch_size =  config.training.batch_sizes
    lr = config.training.learning_rates
    loss_name = config.training.loss_functions

    # build dataset + transforms
    data_transform = transforms.Compose([Normalize(), MinimumToZero()])


    print(f" Trying batch_size={batch_size}, lr={lr}, loss_fn={loss_name}")
    ...
    trainer = Trainer(
        ...
        save_path=config.save_path, log_path=config.log_path,
        dataloader_args={'batch_size': batch_size, 'shuffle': True, 'num_workers': 4},
    ...
    print(f" Finished with val_loss={trainer.lowest_val_loss}")
