...
def sample_model_params(trial, config):
    filters_idx = trial.suggest_int("filters_idx", 0, len(config.model.filters_options) - 1)
    filters = config.model.filters_options[filters_idx]
    kernel_size = config.model.kernel_size_options[filters_idx]
    return {
